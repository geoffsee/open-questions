...
export function getProblemRisk(category: string, section: string, text: string, enrichment?: EnrichmentProblem | null): ProblemRisk {
  const material = [category, section, text, enrichment?.summary, enrichment?.significance, enrichment?.field]
    .filter(Boolean)
    .join(" ");
  ...
  if (/existential|catastrophic|extinction|global|civilization|security/i.test(material)) score += 12;
  if (/human|society|policy|health|medicine|energy|food|water/i.test(material)) score += 7;
